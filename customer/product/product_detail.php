					...
					<?php
						$sqllq="SELECT *
                            ...
                            WHERE id_san_pham<>'".$_GET["masanpham"]."' 
                                AND ABS(don_gia-".$row["don_gia"].") < 100000 AND id_danh_muc='".$row["id_danh_muc"]."' 
                            LIMIT 5";
					...
					<?php
						$sqllq="SELECT *
                            ...
                            WHERE tag in ('".$row["tag_2"]."','".$row["tag_3"]."','".$row["tag"]."')  
                                OR tag_2 in ('".$row["tag_2"]."','".$row["tag_3"]."','".$row["tag"]."') OR tag_3 in ('".$row["tag_2"]."','".$row["tag_3"]."','".$row["tag"]."') ORDER BY don_gia_dv
                            LIMIT 5";
						$resultlq=$con->query($sqllq);
						if($resultlq->num_rows>0)
						{
							...
							while($rowlq= $resultlq->fetch_assoc())
							{
								...
								<div class="thanhPhan <?php if($i==1) echo "s2"; ?>">
									...
									<?php
										if (isset($_SESSION["id-user"]))
										{
											$sqlcheckyeuthich="select * from tbl_yeu_thich_dich_vu where id_khach_hang='".$_SESSION["id-user"]."' and id_dich_vu='".$rowlq["id_dich_vu"]."'";
											$resultyt=$con->query($sqlcheckyeuthich);
											if($resultyt->num_rows>0)
											{
										?>
